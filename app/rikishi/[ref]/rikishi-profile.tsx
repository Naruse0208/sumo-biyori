...
function EloChart({ points }: { points: RatingPoint[] }) {
  ...
  useEffect(() => {
    ...
    const minimum = Math.floor((Math.min(...values) - 30) / 50) * 50;
    const maximum = Math.ceil((Math.max(...values) + 30) / 50) * 50;
    const range = Math.max(100, maximum - minimum);
    const x = (index: number) => padding.left + (index / Math.max(1, points.length - 1)) * chartWidth;
    const y = (elo: number) => padding.top + (1 - (elo - minimum) / range) * chartHeight;
    ...
    points.forEach((point, index) => {
      if (index === 0) context.moveTo(x(index), y(point.elo));
      else context.lineTo(x(index), y(point.elo));
    });
    ...
    context.beginPath();
    context.arc(x(points.length - 1), y(last.elo), 5, 0, Math.PI * 2);
  ...
  return (
    <div className="rikishi-chart-wrap">
      <canvas ref={canvasRef} className="rikishi-chart" aria-label={`${points[0]?.bashoId ?? ""}から${points.at(-1)?.bashoId ?? ""}までのElo推移`} />
      <p className="sr-only">開始時 {points[0]?.elo}、現在 {points.at(-1)?.elo}、最高 {Math.max(...points.map((point) => point.peakElo))}</p>
    </div>
  );
}
    ...
    <main className="rikishi-page">
      ...
      <section className="rikishi-profile-hero">
        ...
        <div className="rikishi-rating-seal">
          <span>現在Elo</span><strong>{latest?.elo ?? "—"}</strong><small>最高 {peakElo}</small>
        </div>
      </section>

      <section className="rate-shell rikishi-summary" aria-label="力士概要">
        <div><span>土俵偏差値</span><strong>{latest ? (latest.dohyoScoreTenths / 10).toFixed(1) : "—"}</strong></div>
      ...
      <section className="rate-shell rikishi-history" aria-labelledby="elo-history-title">
        <div className="rate-section-heading">
          <div><p>RATING HISTORY</p><h2 id="elo-history-title">Elo推移</h2></div>
        ...
        {chartPoints.length > 1 ? <EloChart points={chartPoints} /> : <p className="rikishi-no-chart">推移を描くための場所数がまだ足りません。</p>}
        <div className="rikishi-chart-stats">
          <span>初回 <b>{chartPoints[0]?.elo ?? "—"}</b></span>
          <span>現在 <b>{chartPoints.at(-1)?.elo ?? "—"}</b></span>
